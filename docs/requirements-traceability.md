...
| FR1-5 模板 | `files.py` + `templates.py` + `TemplateFile` | 分类展示、真实上传与下载 |
| FR2 党团 | `party.py` + `PartyStage`/`PartyTimelineRule` | 阶段入库/可配置、时间线、自动提醒、全员进度列表 |
| FR2-6/7 理论 | `theory.py` | 题库 CRUD/导入、随机抽题、判分记录、每日答题限次 |
| FR3 通知 | `notices.py` + `message_channels.py` | 定向规则、批次统计、站内+邮件(SMTP)+短信模拟 |
...
| NFR 认证/审计 | `auth.py`, `audit_logs`, `file_access.py` | JWT+密码、refresh、操作留痕、审计 CSV、附件业务鉴权 |
...
**部署前置**：Kingbase 切换 `DATABASE_URL`（见 `scripts/probe-kingbase.py`）；邮件配 `SMTP_*`；本地 `scripts/setup-local.ps1` 或 `pip install -r backend/requirements.txt` 后 `PYTHONPATH=backend python -m app.seed`。

**Mock/Remote**：新增接口已在 `web/src/api/mockGateway.js` 同步契约，便于离线调试。
